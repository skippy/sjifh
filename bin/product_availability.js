...
const retrieveOrderData = async (page) => {
  ...
  if (argv.continue && fs.existsSync(filename)) {
    ...
    const lastIdFound = parseInt(lastLine.split(',')[0])
  ...
  const orderPeriods = {}
  const orderSubPeriods = {}
  const orderDistributionLocs = {}
  ...
  while (true) {
    ...
    const orderIds = await page.$$('#content table.sticky-table-header tr td:nth-child(2)')
    for (let i = 0; i < orderIds.length; i++) {
      const orderId = (await orderIds[i].innerText()).trim()
      if (orderId === '') continue
    ...
    if (_.isEmpty(orderPeriods)) {
      ...
      const orderPeriodIds = _.keys(orderPeriods)
    ...
    writableStream.cork()
    const items = orderJSON.items ? orderJSON.items : orderJSON.cartItems
...
const retrieveProductAvailabilityData = async (page) => {
  ...
  page.on('response', async (response) => {
    ...
    const periodDate = productDataJSON.items[0].period.trim()
    // productId: across producers...
    let prevProductAType = productDataJSON.items[0]
    const trimmedItems = _.filter(productDataJSON.items, function (currObj) {
  ...
  for (let i = 2; i < allPeriods.length; i++) {
    const periodDate = await allPeriods[i].textContent()
    logger.info('period: ', periodDate)
    const periodId = await allPeriods[i].getAttribute('value')
    try {
